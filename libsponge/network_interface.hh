...
class NetworkInterface {
  private:
    ...
    std::list<std::pair<InternetDatagram, Address>> _frames_wait{};

    struct arp_info {
      EthernetAddress _ethernet_address;
      size_t ttl;
      arp_info(EthernetAddress addr):_ethernet_address(addr), ttl(30000){}
    };
    std::map<uint32_t, arp_info> arp_map{}; // map ip address -> ethernet address
    std::map<uint32_t, size_t> arp_request{};
